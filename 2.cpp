...
char Getchar(){															//use string to storage the input 
	string Gchar;
	while (1){
		try {
			cin>>Gchar;
			if(Gchar!="Y" && Gchar!="y" && Gchar!="N" && Gchar!="n")	//if the input is not Y y N n, then throw 1
				throw 1;
			else return Gchar[0];										//if input is legal, then return
		}
		catch (int i){													//if catch 1, then let user input again
...
double u_turn(int cdn_u[2]){											//user turn
	...
	while (true) {														//giving cards process
		...
		judge = Getchar();												//give one card if user wants 
